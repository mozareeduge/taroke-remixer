...
export function createUndoMiddleware(): Middleware<object, RootState> {
  let lastLabel = "";
  let lastTimestamp = 0;

  return (api) => (next) => (action) => {
    if (mutateProject.match(action)) {
      const { present, label, patches, inversePatches, skipHistory } = action.payload as {
      ...
      // undo/redo restore dispatches mutateProject with skipHistory=true to avoid
      // re-entering this branch and corrupting the future stack.
      if (skipHistory) {
    ...
    if (popForUndo.match(action)) {
      ...
      const result = next(action);
      api.dispatch(mutateProject({ present: reverted as typeof state.project.present, label: `Undo: ${entry.label}`, patches: [], inversePatches: [], skipHistory: true }));
      api.dispatch(markPreviewStale());
      return result;
    }
    ...
    if (popForRedo.match(action)) {
      ...
      const result = next(action);
      api.dispatch(mutateProject({ present: reapplied as typeof state.project.present, label: `Redo: ${entry.label}`, patches: [], inversePatches: [], skipHistory: true }));
      api.dispatch(markPreviewStale());
      return result;
    }
