(function(){
  ...
  function lineInspector(){ const e=ui.events.find(x=>x.id===ui.selectedEvent); if(!e)return ''; const toks=Object.values(e.selectedTokens||{}).filter(Boolean); return `<div class="modalShade" data-close-event><div class="modal" role="dialog" aria-modal="true" aria-labelledby="modal-insp-title" onclick="event.stopPropagation()"><div class="modalHead"><div><div class="panelTitle" id="modal-insp-title">Line recipe</div><div class="panelKicker">${h(e.id)} / ${h(e.deviceName)} / ${h(e.route)}</div></div><button class="btn" data-close-event>Close</button></div><div class="modalBody"><div class="stage"><p class="poemText">${h(e.surface)}</p><p class="micro">${h(e.trace)}</p></div><h3 class="mono">Selected samples</h3><div class="tokenList">${toks.map(t=>`<button class="tokenCell" data-jump-token="${t.id}">${h(t.literal)}</button>`).join('')}</div><div class="row"><button class="btn primary" data-note="keep:${e.id}">Keep as sample line</button><button class="btn" data-note="repair:${e.id}">Mark for repair</button><button class="btn" data-open-device="${e.deviceId}">Open device</button></div></div></div></div>`; }
